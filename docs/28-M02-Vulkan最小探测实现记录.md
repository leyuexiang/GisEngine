...
本文记录 `T-VK-001` 的第一步：在不引入窗口、交换链和渲染管线的前提下，完成 Vulkan 运行时加载、实例创建和物理设备枚举。
...
- 不依赖 Vulkan SDK 头文件或静态导入库，避免 SDK 缺失阻断 Null RHI 和无 GPU 自动化测试。
...
- 当前仅读取 Vulkan 1.x `VkPhysicalDeviceProperties` 的稳定字段，未创建逻辑设备、队列、表面或交换链。
- 下一步需要在确定窗口抽象和 SDK 依赖策略后，增加逻辑设备/队列选择，再接入交换链与清屏通道。

## 4. 验收证据

```text
ctest --test-dir out/build/vs-ninja-debug -R VulkanInstanceTest --output-on-failure
2/2 tests passed
```
